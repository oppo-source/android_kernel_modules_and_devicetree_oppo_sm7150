...
static int fill_boost_page_pool(struct device *dev, struct ion_page_pool *pool)
{
	struct page *page;
	...
	while (true) {
		...
		for (i = 0; i < NUM_ORDERS; i++) {
			while (boost_pool_nr_pages(pool) < pool->low) {
				...
				if (fill_boost_page_pool(pool->dev,
							 pool->pools[i]) < 0)
					break;
			}
		}
					 ...
					 char *name)
{
	...
	if (ion_system_heap_create_pools(boost_pool->pools,
					 ion_flag == ION_FLAG_CACHED, boost_flag))
		goto free_heap;

	boost_pool->origin = boost_pool->high = boost_pool->low = nr_pages;
	boost_pool->name = name;
	boost_pool->usage = ion_flag;
	boost_pool->dev = heap->heap.priv;
