...
extern bool set_limit_core_use_target_cpu(struct task_struct *task, int *target_cpu);
static inline int get_limit_core_use(void)
{
	return DEFAULT_LIMIT_CORE_USE;
}

extern bool is_limit_task(struct task_struct *task);
static inline bool should_limit_core_use(struct task_struct *task)
{
	return task && is_limit_task(task);
}

static inline bool should_limit_task_skip_cpu(struct task_struct *task, int cpu)
{
	return should_limit_core_use(task) && (cpu >= get_limit_core_use());
}
#endif
